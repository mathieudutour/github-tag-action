...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const previousTag = semver_1.parse(tag.name);
            const commits = yield getCommits(githubToken, tag.commit.sha);
            if (!previousTag) {
            ...
            console.log(semver_1.prerelease(previousTag));
            console.log(previousTag, releaseType, appendToPreReleaseTag, currentBranch);
            let incrementedVersion = semver_1.inc(previousTag, releaseType, appendToPreReleaseTag ? appendToPreReleaseTag : currentBranch);
            if (!incrementedVersion) {
                core.setFailed('Could not increment version.');
                return;
            }
            let validVersion = semver_1.valid(incrementedVersion);
            if (!validVersion) {
                incrementedVersion = incrementedVersion.replace(/\//g, '-');
                validVersion = semver_1.valid(incrementedVersion);
                if (!validVersion) {
