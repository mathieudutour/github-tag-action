...
const isReleaseType = (value) => VALID_RELEASE_TYPES.has(value);
async function main() {
    ...
    if (customTag) {
        ...
        const previousVersion = (0, semver_1.parse)(previousTag.name.replace(prefixRegex, ''));
        if (!previousVersion) {
        ...
        const releaseType = candidateReleaseType;
